...
class Mammal {
    let consumptionClasification: ConsumptionClassification = .omnivore
    var health: Health = .healthy
    
    func consume(food: Food){
        //Optional health
//        var newHealth: Health?
//        if (consumptionClasification.canEat(food: food)){
...
//        }
//
        // guard will unwrap our optional and put into a new variable
        guard let newHealth = consumptionClasification.canEat(food: food) ? health.increasedHealth : health.decreasedHealth  else {
            return
        }
...
let animal = Mammal()
animal.consumptionClasification
animal.health
var food: Food = .chicken
animal.consume(food: food)
...
class Human : Mammal {
    ...
    override func consume(food: Food) {
        guard let newHealth = consumptionClasification.canEat(food: food) && !allergies.contains(food) ? health.increasedHealth : health.decreasedHealth  else { return }
        health = newHealth
//        for item in allergies {
//            if item == food {
//                super.health.decreasedHealth
//            }
//        }
    }
...
var allergies = [Food.chocolate]
let amanda = Human(foodArray: allergies)
...
final class Child : Human {
    ...
    init(dislikedFoods: [Food], allergies: [Food]) {
        ...
        super.init(foodArray: allergies)
        }
    
    // cannot override health since base class is using final specifier
